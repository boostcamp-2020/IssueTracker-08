const query = {
  ...
  GET_ASSIGNEES_BY_ISSUE_ID: `SELECT issue_assignee.userId,
    (SELECT user.imageUrl FROM user WHERE user.id = issue_assignee.userId) as imageUrl,
  ...
  GET_COMMENTS: `SELECT comment.id as commentId, 
    comment.userId as userId,
    (SELECT user.name FROM user WHERE user.id = comment.userId) as name, 
    (SELECT user.imageUrl FROM user WHERE user.id = comment.userId) as imageUrl, 
    comment.content, comment.createAt 
    FROM comment 
    WHERE comment.issueId = ?`,
  GET_COMMENTS_COUNT:
